...
class _PrivacyScreenState extends State<PrivacyScreen> {
  ...
  Future<void> fetchData() async {
    try {
      ...
      // استرجاع البيانات من SharedPreferences إذا كانت موجودة
      SharedPreferences prefs = await SharedPreferences.getInstance();
      String? cachedData = prefs.getString('descriptions');

      if (cachedData != null) {
        // إذا كانت البيانات موجودة في SharedPreferences، استخدمها مباشرة
        print("Data loaded from cache: $cachedData");
        setState(() {
        ...
        // إذا لم تكن البيانات موجودة في SharedPreferences، قم بتحميلها من API
        var response = await Dio().get(
          '$baseUrl/api/all-terms',
        ...
        if (response.statusCode == 200) {
          ...
          await prefs.setString('descriptions', descriptionsList.join(','));
